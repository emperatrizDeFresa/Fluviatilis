...
abstract class BaseSettingsActivity : Activity() {
    ...
    override fun onResume() {
        ...
        if (isWallpaperSupported()) {
            settings_set_wallpaper.visibility = //if (isThisAppTheSelectedWallpaper()) View.GONE else
                View.VISIBLE
        }
    }
    ...
    private fun positionViewsInsideSystemViews() {
        if (Build.VERSION.SDK_INT >= Build.VERSION_CODES.LOLLIPOP) {
            window.decorView.setOnApplyWindowInsetsListener { _, insets ->
                run {
                    val params = settings_layout.layoutParams
                            ?: RelativeLayout.LayoutParams(RelativeLayout.LayoutParams.WRAP_CONTENT, RelativeLayout.LayoutParams.WRAP_CONTENT)
                    (params as RelativeLayout.LayoutParams).topMargin = insets.systemWindowInsetTop
                    (params as RelativeLayout.LayoutParams).bottomMargin = insets.systemWindowInsetBottom
                    settings_layout.layoutParams = params
                }
